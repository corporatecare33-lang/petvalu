...
Route::get('/public/{path}', function (string $path) {
    ...
    abort_unless(
        $basePath && $filePath && str_starts_with($filePath, $basePath) && is_file($filePath),
        404
    );
...
Route::get('/test-imgurl', function () {
    $testPath = 'public/uploads/category/1773432503-1770953958-1767432776-pngtree-smart-android-tv-png-image_16548885.webp';
    
    echo '<h1 style="text-align: center; margin-top: 20px;">Testing imgUrl Function</h1>';
    echo '<div style="max-width: 800px; margin: 0 auto; padding: 20px; font-family: Arial;">';
    echo '<h2>Debug Info:</h2>';
    echo '<ul style="list-style-type: none; padding: 0;">';
    echo '<li><strong>APP_URL:</strong> ' . env('APP_URL') . '</li>';
    echo '<li><strong>Test Path (from DB):</strong> ' . $testPath . '</li>';
    $cleanPath = $testPath;
    if (strpos($cleanPath, 'public/') === 0) {
        $cleanPath = substr($cleanPath, 7);
    }
    echo '<li><strong>Cleaned Path:</strong> ' . $cleanPath . '</li>';
    echo '<li><strong>asset() Output:</strong> ' . asset($cleanPath) . '</li>';
    echo '<li><strong>url() Output:</strong> ' . url($cleanPath) . '</li>';
    echo '<li><strong>imgUrl() Output:</strong> ' . imgUrl($testPath) . '</li>';
    echo '<li><strong>Full File Path:</strong> ' . public_path($cleanPath) . '</li>';
    echo '<li><strong>File Exists?</strong> ' . (file_exists(public_path($cleanPath)) ? '<span style="color: green; font-weight: bold;">YES</span>' : '<span style="color: red; font-weight: bold;">NO</span>') . '</li>';
    echo '</ul>';
    
    echo '<hr>';
    echo '<h2>Image Previews:</h2>';
    
    echo '<div style="margin-bottom: 30px;">';
    echo '<h3>1. Using imgUrl():</h3>';
    echo '<img src="' . imgUrl($testPath) . '" style="max-width: 500px; border: 1px solid #ddd; padding: 10px;">';
    echo '</div>';
    
    echo '<div style="margin-bottom: 30px;">';
    echo '<h3>2. Using asset($cleanPath):</h3>';
    echo '<img src="' . asset($cleanPath) . '" style="max-width: 500px; border: 1px solid #ddd; padding: 10px;">';
    echo '</div>';
    
    echo '<div style="margin-bottom: 30px;">';
    echo '<h3>3. Using hardcoded URL:</h3>';
    $hardcodedUrl = (env('APP_URL') ? env('APP_URL') : 'http://127.0.0.1:8000') . '/' . $cleanPath;
    echo '<img src="' . $hardcodedUrl . '" style="max-width: 500px; border: 1px solid #ddd; padding: 10px;">';
    echo '<p style="margin-top: 10px;">Hardcoded URL: ' . $hardcodedUrl . '</p>';
    echo '</div>';
    echo '</div>';
});
