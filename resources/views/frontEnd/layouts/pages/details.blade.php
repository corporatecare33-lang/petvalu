...
@php
    ...
    $metaImage = $details->meta_image ? imgUrl($details->meta_image) : imgUrl(optional($details->image)->image);
@endphp
                                ...
                                <div class="details_slider owl-carousel" id="details_slider_main">
                                    @foreach ($details->images as $value)
                                        <div class="dimage_item" data-color-id="{{ $value->color_id ?? '' }}">
                                            <img src="{{ imgUrl($value->image) }}" class="block__pic" />
                                        </div>
                                    @endforeach
                                </div>
                                <div
...
<script>
    ...
    @php
        $productImagesForJs = $details->images->map(function($img) {
            $cleanPath = $img->image;
            if (strpos($cleanPath, 'public/') === 0) {
                $cleanPath = substr($cleanPath, 7);
            }
            return ['src' => asset($cleanPath), 'color_id' => $img->color_id];
        });
    @endphp
    var productImages = @json($productImagesForJs);
